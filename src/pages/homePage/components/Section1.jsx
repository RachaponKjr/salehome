        ...
        {tabs === 0 && (
          <Grid gridTemplateColumns={{ base: "repeat(2, minmax(0, 1fr))", sm: "repeat(3, 1fr)", lg: "repeat(4, 1fr)" }} gap={{ base: "4px", md: "16px" }}>
            {mummyData.map((item, index) => (
              <GridItem w={"100%"} p={{ base: "4px", md: "8px" }} key={index}>
                <Flex w={"100%"} h={"100%"} flexDirection={"column"} justifyContent={"space-between"} overflow={"hidden"} borderRadius={"10px"} boxShadow={'md'}>
                  {/* รูปภาพใน card */}
                  ...
                  <Flex flexDirection={"column"} zIndex={10} gap={"4px"} py={1} px={{ base: 1, md: 2 }} bg={"#FAFAFA"} >
                    {/* หัวข้อของสินค้า */}
                    <Link href={"/" + item.id} fontSize={{ base: "12px", md: "14px" }} cursor={"pointer"} className='line-clamp'>{item.title}</Link>
